...
to-report select-action-e-greedy [x y]
  ...
  [let action-values (array:to-list (get-Q-values x y ))
    let max-value-idx (position (max action-values) action-values)
    report (item max-value-idx ACTION-LIST)
  ]
end
...
to-report select-action-soft-max [x y]
  let action-values array:to-list (get-Q-values x y)
  let action-probs (map [ -> (exp (q/temperatura))] action-values)
  ...
  [set action-index (action-index + 1)
    set prob-sum (prob-sum + (item action-index action-prob))
  ]
